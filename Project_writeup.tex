...
Consider the case when our lattice $L = O_{D}$ for some discriminant $D$, where the basis elements will be $[1, \frac{1+\sqrt{D}}{2}] $ or $[1, \sqrt{D}]$ depending on whether $D$ is $\{1\}, \{2, 3\} \mod 4$ respectively, which are quadratic integer fields. 
